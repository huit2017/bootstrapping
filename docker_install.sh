...
[Service]
ExecStart=
ExecStart=/usr/bin/dockerd -H tcp://192.168.32.15:2375 -H unix:///var/run/docker.sock
EOT
...
# scp /var/lib/jenkins/.ssh/id_rsa.pub vagrant@192.168.32.15:~/
mv id_rsa.pub ${image_name}/
#sudo docker build -ti --name java8-maven jenkins/${image_name}:1.0 ${image_name}
#sudo docker run -d jenkins/${image_name}:1.0
